...
export default class App extends Component {
    ...
    this.grabProductInfo = this.grabProductInfo.bind(this);
    this.handleRelatedItemClick = this.handleRelatedItemClick.bind(this);
  }

  componentDidMount() {
    this.grabProductInfo();
  }
  ...
  handleRelatedItemClick(ID) {
    let getProduct =
    `http://3.21.164.220/products/${ID}`;
    let getProductStyles =
    `http://3.21.164.220/products/${ID}/styles`;
    let getProductQuestions =
    `http://3.21.164.220/qa/questions?product_id=${ID}`;
    let getProductReviews =
    `http://3.21.164.220/reviews?product_id=${ID}`;
    let getMetaData =
    `http://3.21.164.220/reviews/meta?product_id=${ID}`;
    let getRelatedItems =
    `http://3.21.164.220/products/${ID}/related`;
    ...
    axios
      ...
      .then(
        axios.spread((...responses) => {
          ...
          console.log('THIS IS NEW PRODUCT ID ', this.state.productID)
        })
      )
      // .catch(errors => {
      //   // react on errors.
